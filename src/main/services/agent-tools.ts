...
export function makeTools(emit: Emit, client: Anthropic, workDir: string) {
  ...
  return [
    ...
    betaTool({
      name: 'activate_skill',
      get description() {
        // תיקון: גישה למערך דרך SkillsStore.items
        const catalog = SkillsStore.items.map(
          (s) => `- Name: "${s.name}"\n  Description: ${s.description}`
        ).join('\n');
        return (
          'Retrieves the full instructions and prompt details for a specific skill.\n\n' +
          'Available Skills Catalog:\n' +
          `${catalog || 'No skills loaded.'}\n\n` +
          'Instruction: If the user workflow or task matches one of the skills listed above, ' +
          'you MUST call this tool with the exact skillName to read its full operational principles ' +
          'before proceeding with the user request.'
        );
      },
      inputSchema: {
        type: 'object',
        properties: {
          skillName: {
            type: 'string',
            description: 'The exact name of the skill to activate (e.g., "excel-expert").'
          }
        },
        required: ['skillName'],
        additionalProperties: false
      ...
      run: async ({ skillName }) => {
        const id = start('activate_skill', { skillName });
        
        // תיקון: שימוש במתודה find שמוגדרת באובייקט SkillsStore
        const skill = SkillsStore.find((s) => s.name === skillName);
        if (!skill) {
          fail(id, `not found: ${skillName}`);
          return `Error: Skill "${skillName}" is not available. Check the catalog for valid names.`;
        }

        ok(id, `activated: ${skillName}`);
        return skill.body;
      }
    })
  ];
